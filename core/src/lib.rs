    ...
    pub async fn start(mut cfg: config::Config, path: &str) -> Result<Arc<Self>, String> {
        ...
        if cfg.tunnel_mode {
            match tunnel_service::state_in(&tdir) {
                ...
                // ⛔ Ставим, а не жалуемся в журнал. Настройки переживают
                // удаление программы: после переустановки перехват в них
                // включён, службы нет, и раньше программа молча писала об
                // этом в журнал — человек видел «ничего не работает» и
                // никакого способа это исправить.
                tunnel_service::State::Absent => {
                    logfile::line(&logfile::now_stamp(),
                        "перехват включён в настройках, службы нет — ставлю");
                    match tunnel_service::install(&tdir)
                        .and_then(|_| tunnel_service::start_in(&tdir))
                    {
                        Ok(_) => logfile::line(&logfile::now_stamp(), "перехват: поднят при запуске"),
                        Err(e) => logfile::line(&logfile::now_stamp(),
                            &format!("перехват при запуске не поднялся: {e}")),
        ...
        if !stale.is_empty() {
            logfile::line(&logfile::now_stamp(),
                &format!("убраны переменные окружения, оставшиеся от прежних версий: {}",
                         stale.join(", ")));
        }
