...
pub fn install(dir: &Path) -> Result<(), String> {
    ...
    // больше не попытавшись её поставить. Проверяем по делу: копия
    // программы должна оказаться на своём месте.
    if !runner_path(dir).is_file() {
        crate::logfile::line(&crate::logfile::now_stamp(),
            &format!("перехват: копия программы не появилась: {}", runner_path(dir).display()));
        return Err(format!("служба поставлена не полностью: не появилась копия {}. \
