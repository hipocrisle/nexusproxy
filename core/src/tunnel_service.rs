...
mod imp {
    ...
    /// планировщик нельзя — смотрим на сам engine.
    ...
    pub fn state_in(dir: &Path) -> State {
        ...
        let flag = super::flag_path(dir).exists();
        let engine = task_running(dir);
        if flag && engine {
            State::Running
        } else {
    ...
    /// ⛔ Ставим flag, а не запускаем задачу. Задача принадлежит
    ...
    pub fn start_in(dir: &Path) -> Result<(), String> {
        ...
        // Задача крутится с включения компьютера и сама поднимет engine
        ...
        // Служба поднимет engine сама, flag она уже видит.
        for _ in 0..6 {
        ...
        if task_exists() {
            // Не ошибка: служба есть, flag стоит, движку нужно время.
            Ok(())
        } else {
...
mod imp {
    ...
    /// перехват поднят, файл удалён — остановлен. Сам flag общий для
    ...
    pub fn state_in(dir: &Path) -> State {
        ...
        // ⛔ Одного признака мало: он лежит на месте и когда engine не
    ...
    /// flag-файл, за которым следит наша обёртка.
    ///
    ...
    /// engine не поднимает. Так что «работает постоянно» — это про
...
# Есть flag — engine работает, нет — стоит.
BIN="{bin}"
CFG="{cfg}"
FLAG="{flag}"
LOG="{log}"
...
"#, watcher_path(dir).display(), dir.join("daemon.log").display())
    ...
    pub fn start_in(dir: &Path) -> Result<(), String> {
        ...
        // flag не меняется, launchd ничего не перезапускает — и
        // engine продолжает жить со старым списком приложений. Человек
        // добавил Cursor, а трафик идёт мимо.
        //
        // ⛔ Снять engine своими силами нельзя: он работает от системы, а
        // мы — от пользователя, прав не хватает (в журнале было «снято:
        // 0»). Поэтому перезапускаем признаком: убрали файл — launchd
        // остановил engine, вернули — поднял заново, уже с новыми
        // настройками.
        if was_on {
...
/// Включить. На macOS — создать файл-flag, на Windows — запустить задачу.
pub fn start_in(dir: &Path) -> Result<(), String> {
    imp::start_in(dir)
}
...
mod tests {
    ...
    /// администратора engine падает на создании интерфейса — «configure
    ...
    /// ⛔ Сам по себе перехват не включается — engine поднимается только
...
/// Файл-flag: есть — перехват включён. ⛔ Через него, а не через
...
/// обычными правами и engine падает с «configure tun interface:
...
/// ⛔ Задача запускает НАС, а не engine напрямую: сама она окно
...
<!-- ⛔ Задача поднимается при включении компьютера и работает
     ...
     выключение идут через файл-flag, который пишется обычными
     правами. -->
<BootTrigger>
