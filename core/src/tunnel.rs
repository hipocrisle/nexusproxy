...
//! работает на Windows и macOS. Писать ours поверх драйверов не нужно.
//!
...
///
/// На macOS `.app` — folder со всем хозяйством приложения, и её путь
/// годится целиком. На Windows procs_at приложения лежат в своей папке,
/// поэтому берём её.
fn path_prefix(path: &str) -> Option<String> {
    ...
    // «D:» — и под правило попадали ВСЕ procs_at с этого диска, то present
    ...
    let is_root = folder.is_empty()
        || folder.ends_with(':')
        || folder.chars().all(|c| c == '/' || c == '\\');
    if is_root {
        return None;
    }
...
///
/// ⛔ Два правила обязаны идти ПЕРЕД остальными: private_at адреса и адрес
...
pub struct DomainRule {
    /// Образец: domain_at или subnet_at.
    pub pattern: String,
    /// Тег прокси.
    pub via: String,
}
...
pub fn build_config_with(
    ...
    // локальный port, помощник идёт к своему серверу, и тот же туннель
    // отправляет его обратно в тот же port.
    //
    // ⛔ Правило ставится ТОЛЬКО когда страны present. Без них петле взяться
    // неоткуда, а лишнее «напрямую» сломало бы работу там, где выход в
    // сеть present только через корпоративный прокси.
    let has_countries = upstreams.iter().any(|u| u.address == "127.0.0.1");
    if has_countries {
        // Берём всю папку программы, а не только папку перехвата:
        // помощник лежит в соседней.
        let ours: Vec<String> = dir_hint.parent()
            .map(|d| vec![path_regex(&d.to_string_lossy())])
            .unwrap_or_default();
        if !ours.is_empty() {
            rules.push(serde_json::json!({
                "process_path_regex": ours,
                "action": "route",
                "outbound": "direct"
            }));
        }
    ...
    // port 53 обычно запрещает, — имя не разрешалось, и программа
    ...
    // требует совпадения всех условий сразу, а у приложения procs_at
    ...
    serde_json::json!({
        ...
        // запросы сам, и внутренние names_at — контроллеры домена, файловые
...
mod tests {
    ...
    fn очерёдность_правил_не_глушит_нужное() {
        ...
        let at = |ищем: &dyn Fn(&serde_json::Value) -> bool| -> usize {
            rules.iter().position(|r| ищем(r)).unwrap_or(usize::MAX)
        };
        let udp = at(&|r| r["network"] == "udp");
        let names_at = at(&|r| r["port"].as_array().map(|a| a.contains(&serde_json::json!(53))).unwrap_or(false));
        let procs_at = at(&|r| r.get("process_name").is_some());
        let private_at = at(&|r| r["ip_is_private"] == true);
        let subnet_at = at(&|r| r["ip_cidr"].as_array()
            .map(|a| a.iter().any(|v| v == "10.10.0.0/16")).unwrap_or(false));

        assert!(udp < procs_at, "UDP выбранной программы уйдёт в прокси, а через него он не ходит");
    ...
    /// локальный port, и получается круг.
    #[test]
    fn при_странах_своё_хозяйство_идёт_напрямую() {
        let country = Upstream {
        ...
        let rules = rules_of(&c);
        let ours = rules.iter().position(|r| r["process_path_regex"].as_array()
    ...
    fn без_стран_лишнего_правила_нет() {
        ...
        let present = rules_of(&c).iter().any(|r| r["process_path_regex"].as_array()
            .map(|a| a.iter().any(|v| v.as_str().unwrap_or("").contains("nexusproxy")))
            .unwrap_or(false));
        assert!(!present, "лишнее правило «напрямую» сломает работу через корпоративный прокси");
    }
    ...
    fn падающий_по_кругу_движок_не_считается_живым() {
        ...
        std::fs::write(api_path(&dir),
            serde_json::to_vec(&Api { port: port, secret: "x".repeat(32) }).unwrap()).unwrap();
...
pub fn is_alive(dir: &Path) -> bool {
    ...
    // «перехват работает» при полностью мёртвом туннеле, то present ровно
...
pub fn write_config(dir: &Path, routes: &[Route], domains: &[DomainRule],
    ...
    // Какие именно procs_at ловим — самое важное для разбора: если имя
...
/// Следить за признаком включения и держать движок, пока он present.
///
...
/// Поэтому следим сами: present признак — движок работает и перечитывает
...
pub fn watch_flag(dir: &Path, flag: &Path) -> Result<(), String> {
    ...
    loop {
        ...
        // Без обрезки он съедает at, а программа виснет, читая его.
        tick += 1;
        if tick % 60 == 0 {
            trim_log(dir);
        }
...
/// private_at счётчики остаются пустыми, и человек видит нули при работающем
/// туннеле. Движок ведёт учёт сам — domain_at, программу, трафик в обе
...
/// listener он на своей машине: без пароля любая программа на компьютере
...
pub fn api_access(dir: &Path) -> Api {
    ...
    // ⛔ Если записать не удалось, следующий вызов выдумает новые port и
...
///
/// ⛔ Занятый посторонней программой port роняет движок ЦЕЛИКОМ:
...
/// же движок. Меняя port при каждой проверке, мы переписывали бы
/// настройки и перезапускали движок по кругу. Поэтому спрашиваем: если
/// на нашем пароле отвечают — это он и present.
...
mod path_tests {
    ...
    /// На Windows procs_at приложения лежат в своей папке — берём её,
    /// а не сам файл, иначе вспомогательные procs_at снова пройдут мимо.
    #[test]
...
/// Полная картина: что present на диске, что работает, что в журналах.
///
...
pub fn diagnosis(dir: &Path) -> String {
    ...
    // ⛔ Какие procs_at движок РЕАЛЬНО увидел. Если приложения тут нет,
...
mod dns_tests {
    ...
    fn запросы_имён_идут_мимо_нас() {
        ...
        // кругу, и внутренние names_at перестали бы разрешаться.
...
/// корпоративного сервера, и внутренние names_at разрешались неверно.
...
pub fn system_resolvers() -> Vec<String> {
    ...
    for line in text.lines() {
        ...
        if let Some(ip) = candidate {
            // ⛔ Свой же адрес в список не берём: это и present петля.
...
/// такого не ждёт и не найдёт, куда делось at.
...
mod api_tests {
    ...
    /// ⛔ Занятый посторонним port роняет движок целиком, и перехват не
    ...
    /// А свой port не трогаем: иначе настройки переписывались бы и
