...
fn path_prefix(path: &str) -> Option<String> {
    ...
    // «D:» — и под правило попадали ВСЕ procs_at с этого диска, то есть
...
pub struct Upstream {
    ...
    /// ⛔ Страна из подписки поднята локальным портом нашего же
    /// помощника. Отличать её по адресу нельзя: корпоративный прокси
    /// тоже бывает местным — переходник, локальный агент.
...
pub fn build_config_with(
    ...
    let has_countries = upstreams.iter().any(|u| u.from_subscription);
    if has_countries {
        // Берём всю папку программы, а не только папку перехвата:
        // помощник лежит в соседней.
        // ⛔ Пустой путь отбрасываем. Для относительного пути настроек
        // родитель — пустая строка, а не «нет родителя», и правило
        // получалось вида «любой путь» — весь трафик уходил напрямую, а
        // перехват молча не делал ничего.
        let ours: Vec<String> = dir_hint.parent()
            .map(|d| d.to_string_lossy().to_string())
            .filter(|d| !d.is_empty() && d != "." && d != "/")
            .map(|d| vec![path_regex(&d)])
            .unwrap_or_default();
        if !ours.is_empty() {
            rules.push(serde_json::json!({
...
/// слушатель он на своей машине: без пароля любая программа на компьютере
    ...
    fn весь_udp_идёт_напрямую() {
        let c = build_config(
            ...
            &[Upstream { tag: "основной".into(), kind: Kind::Socks5,
                         address: "172.31.211.1".into(), port: 1081,
                         user: None, password: None , from_subscription: false }],
        );
