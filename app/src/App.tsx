...
function explainFailure(err: string): string {
  ...
  if (t.includes("служба перехвата не выполняется")) {
    return "Служба перехвата не работает. Переустановите её кнопкой выше — права спросят один раз, дальше перехват включается без них.";
  }
  if (t.includes("права") || t.includes("администратор")) {
    return "Нужны права администратора — один раз, на установку службы. Если их нет, подтверждение введёт ваш системный администратор; дальше перехват работает без прав.";
  }
